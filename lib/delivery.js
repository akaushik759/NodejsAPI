...
function addOrderToFreeDeliveryExecutive(order_id,order_address,next_order_list,assigned_executive_id){
	return new Promise(function(resolve,reject){
	...
	return new Promise((resolve,reject)=>{
    	...
    	time_taken_list = await Promise.all(result.map(async data => await getTimeTakenInSec(data.final_address ? data.final_address:data.location,order_address)));
      	...
      	for(var i=0;i<time_taken_list.length;i++)
			...
			next_order_list = found[4] ?found[4]:[];
    		...
    		{	next_order_list.push([order_id, order_address]);
    			console.log("stringify : "+typeof(JSON.stringify(next_order_list)));
    			addOrderToFreeDeliveryExecutive(order_id,order_address,JSON.stringify(next_order_list),assigned_executive_id)
    			.then((result)=>{

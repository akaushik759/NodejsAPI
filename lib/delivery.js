...
function assignDeliveryExecutive(order_id,order_address){
	return new Promise(function(resolve,reject){
    	...
    	getAllDeliveryExecutives().then((result)=>{
    	const time_taken_list = await Promise.all(result.map(async data => await getTimeTakenInSec(data.final_address,order_address)));
